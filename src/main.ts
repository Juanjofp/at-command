...
const serialLora = '/dev/tty.usbmodem1101';
...
async function runCommand(
    ...
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            reject('Last data received: ' + lastData);
            parser.removeListener('data', parserListener);
        }, 5000);
    });
}
...
// }
async function main() {
    const port = new SerialPort(serialLora, err =>
...
// async function main() {
//     const port = new SerialPort(serialLora, { baudRate: 115200 }, err =>
//         console.log('Serial port opened', err)
//     );
//     const parser = port.pipe(new Readline({ delimiter: '\r\n' }));
