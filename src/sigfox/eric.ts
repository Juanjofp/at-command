...
export function buildEric(
  ...
  async function getVersion() {
    try {
      ...
      logger.error('[Sigfox ERIC]', 'getVersion (AT$I=4,5,8)', `${error}`);
  ...
  async function getInformation() {
    try {
      const responses = await runner.runCommands(getInformationCommands());
  ...
  async function sendData(data: string, { timeout = commandTimeout } = {}) {
    ...
    try {
      await runner.runCommand(generateSendDataCommand(frame, { timeout }));
    } catch (error) {
      logger.error('[Sigfox ERIC]', `sendData (AT$SF=${data},0)`, `${error}`);
  ...
  async function sendDataAndWaitForResponse(
    ...
    try {
      ...
      logger.error('[Sigfox ERIC]', `sendData (AT$SF=${data},1)`, `${error}`);
...
function parseInformation(data: CommandResult[]) {
  ...
  information.region =
    frequency < 900 ? 'EU868' : frequency > 900 ? 'US915' : information.region;
