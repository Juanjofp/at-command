...
> = {
  async list() {
    ...
    const output = ports
      .map(
        (port, index) =>
          `${index} -> ${port.manufacturer || 'Unknown'}: ${port.path}`
      )
      .join('\n');
  ...
  async status(model, portName) {
    ...
    const outputKeys = Object.keys(information) as (keyof typeof information)[];
...
function printMenu() {
  ...
  console.log('  sendWait <RAK811|TD1208|RAK11300|ERIC> <portName> <hexData>');
  console.log('\n----------------------------------------');
  console.log('  Sample: @juanjofp/at-command-cli version rak811 /dev/ttyUSB0');
  console.log('----------------------------------------\n\n');
}
