...
function buildATSerialPort(port: SerialPort): ATSerialPort {
  ...
  function write(data: WriteValues, encoding?: Encodings): Promise<number> {
    return new Promise((resolve, reject) => {
      if (!port.isOpen)
        return void reject(new Error(`Cannot write in a closed serialport`));
      port.flush(function bufferCleaned(errorInflush) {
        if (errorInflush)
          return void reject(
            new Error(`Flush Error for ${data}: Error ${errorInflush}`)
          );
        port.write(data, encoding, errorInWrite => {
          if (errorInWrite)
            return void reject(
              new Error(`Write Error for ${data}: Error ${errorInWrite}`)
            );
          port.drain(function clearBuffer(errorInDrain) {
            if (errorInDrain)
              return void reject(
                new Error(`Drain Error for ${data}: Error ${errorInDrain}`)
              );
            resolve(data.length);
          });
        });
  ...
  async function open(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      port.open(error => {
        if (error)
          return void reject(new Error(`Error opening serialport: ${error}`));
  ...
  async function close(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      port.close(error => {
        if (error)
          return void reject(new Error(`Error closing serialport: ${error}`));
        return void resolve(undefined);
      });
    });
  }
